# Instructions de build — BEGIN Construction

## Objectif

Construire un home page marketing pour **BEGIN Construction** (Entrepreneur général, Gatineau).

**Tu es capable de travail créatif extraordinaire. N'aie pas peur — BEGIN Construction mérite un site qui lui ressemble, pas un template.**
...
Avant d'écrire la moindre ligne de code dans `src/components/sections/*`, écris `decisions.md` à la racine du repo. Ce fichier contient trois blocs courts et spécifiques à **BEGIN Construction** :
...
{1-2 lignes : ce qui rend CETTE section non-générique pour **BEGIN Construction** spécifiquement. Pointer un trait de la phrase d'identité de `decisions.md` qui doit transparaître ici. Pas de mention de layout ni de tokens — ceux-là vivent dans le skill + Hard requirements.}
...
**Google Maps embed** dans Contact (zéro API key requise) :
```html
<iframe src="https://www.google.com/maps?q={encodeURIComponent(adresse)}&output=embed" width="100%" height="400" loading="lazy" />
```
Utilise `lead.json.address` ou `lead.json.zone` comme query. Si zéro adresse, omet.
...
```bash
git add .
git commit -m "Initial build for BEGIN Construction via Replit Agent"
